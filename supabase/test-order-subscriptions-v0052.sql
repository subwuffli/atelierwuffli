-- Nur im Testprojekt ausführen.
-- V0.0.52: Mehrere Abhol-/Liefertermine pro Auftrag.
...
where fulfilment_date is not null and fulfilment_dates='[]'::jsonb;
...
begin
  ...
  result:=public.save_order_v1(p_order||jsonb_build_object('fulfilmentDate',normalized_dates->>0),p_expected_updated_at,p_session_token);
  update public.orders set fulfilment_dates=normalized_dates where id=(result->>'id')::uuid;
  return result;
end $$;
