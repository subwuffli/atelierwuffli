-- Test: globale Positionsvorlagen für alle ERP-Benutzer.
...
alter function public.export_erp_backup() rename to export_erp_backup_v0044;
create function public.export_erp_backup() returns jsonb language sql security invoker set search_path=public as $$
select jsonb_set(public.export_erp_backup_v0044(),'{settings,positionTemplates}',(select position_templates from public.company_settings where id='main'),true);
$$;
...
revoke all on function public.export_erp_backup() from anon;
