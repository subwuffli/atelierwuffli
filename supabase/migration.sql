...
create table if not exists public.invoices (
  ...
  status text not null check (status in ('Offen','Bezahlt','Storniert')), text text not null default '', total numeric(12,2) not null default 0,
  customer_snapshot jsonb not null default '{}'::jsonb, archived boolean not null default false,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
begin
  ...
  on conflict(entity_type,entity_id) do update
    set user_id=excluded.user_id,session_token=excluded.session_token,expires_at=excluded.expires_at
