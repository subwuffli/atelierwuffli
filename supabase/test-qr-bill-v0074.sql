...
select jsonb_build_object(
  ...
  'expenses',coalesce((select jsonb_agg(jsonb_build_object('id',e.id,'number',e.number,'date',e.expense_date,'amount',e.amount,'description',e.description,'createdAt',e.created_at,'updatedAt',e.updated_at) order by e.expense_date,e.created_at) from expenses e),'[]'::jsonb)
);
$$;
