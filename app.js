...
function ordersTable(rows){return rows.length?`<div class="table-wrap"><table><thead><tr><th>Nummer</th><th>Kunde</th><th>Art / Datum</th><th>Status</th><th>Betrag</th><th></th></tr></thead><tbody>${rows.map(o=>`<tr><td><button class="clickable" onclick="printDocument('order','${o.id}')">${esc(o.number)}</button></td><td>${esc(o.customerSnapshot?.name)}</td><td>${esc(o.fulfilment)}<br><span class="muted">${date(o.fulfilmentDate)}</span></td><td><span class="badge ${o.status==='Abgeschlossen'?'ok':'warn'}">${o.status}</span></td><td>${money(o.total)}</td><td><div class="actions"><button class="secondary" onclick="orderForm('${o.id}')">Bearbeiten</button><button class="secondary" onclick="printDocument('order','${o.id}')">PDF / Drucken</button>${!o.invoiceId?`<button class="primary" onclick="createInvoice('${o.id}')">Rechnung</button>`:''}<button class="secondary" onclick="toggleArchive('orders','${o.id}')">${o.archived?'Aktivieren':'Archivieren'}</button></div></td></tr>`).join('')}</tbody></table></div>`:'<div class="card empty">Keine Aufträge vorhanden.</div>'}
