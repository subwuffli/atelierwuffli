...
const blankState=()=>({version:2,revision:0,settings:{name:'',address:'',iban:'',paymentDays:30,logo:'',orderText:'',invoiceText:''},customers:[],orders:[],invoices:[],expenses:[],lastExport:null});
...
async function deleteExpense(id){const x=state.expenses.find(e=>e.id===id);if(!x||!confirm(`Ausgabe "${x.description}" wirklich löschen?`))return;state.expenses=state.expenses.filter(e=>e.id!==id);await save();renderExpenses();notice('Ausgabe gelöscht.')}
...
async function pdfMonthlyReport(kind,month){if(!window.jspdf?.jsPDF){alert('Die PDF-Funktion konnte nicht geladen werden.');return}const isExpense=kind==='expenses',rows=isExpense?state.expenses.filter(x=>monthKey(x.date)===month):state.invoices.filter(i=>i.status==='Bezahlt'&&monthKey(incomeDate(i))===month),total=rows.reduce((s,x)=>s+Number(isExpense?x.amount:x.total),0),{jsPDF}=window.jspdf,doc=new jsPDF({unit:'mm',format:'a4'});doc.setFontSize(20);doc.text(`${isExpense?'Ausgaben':'Einnahmen'} ${monthLabel(month)}`,15,20);doc.setFontSize(10);doc.text(`Erstellt am ${date(today())}`,15,28);let y=42;for(const x of rows){const line=isExpense?`${date(x.date)}  ${x.description}`:`${date(incomeDate(x))}  ${x.number}  ${x.customerSnapshot?.name||''}`;const parts=doc.splitTextToSize(line,140);if(y+parts.length*5>275){doc.addPage();y=20}doc.text(parts,15,y);doc.text(money(isExpense?x.amount:x.total),195,y,{align:'right'});y+=Math.max(8,parts.length*5)}doc.setDrawColor(120);doc.line(15,y,195,y);y+=9;doc.setFontSize(14);doc.text(`Total: ${money(total)}`,195,y,{align:'right'});const file=`${isExpense?'Ausgaben':'Einnahmen'}-${month}.pdf`,standalone=window.matchMedia?.('(display-mode: standalone)').matches||window.navigator.standalone===true;if(standalone){doc.save(file);return}const url=URL.createObjectURL(doc.output('blob')),opened=window.open(url,'_blank');if(!opened)doc.save(file);setTimeout(()=>URL.revokeObjectURL(url),60000)}
...
async function pdfDocument(type,id){
  ...
  if(isInv||isReceipt){
    const rose=[252,229,229],roseStrong=[235,151,151],ink=[54,47,52],muted=[105,86,94],gold=[208,163,66];
    const decorate=()=>{doc.setFillColor(249,241,240);doc.rect(0,0,210,297,'F');doc.setDrawColor(...roseStrong);doc.setLineWidth(.35);doc.roundedRect(7,7,196,283,3,3,'S');doc.setFillColor(...rose);doc.ellipse(177,18,24,12,'F');doc.ellipse(25,278,20,11,'F');for(const [x,y,flip] of [[190,24,-1],[17,272,1]]){doc.setDrawColor(72,63,67);doc.setLineWidth(.35);doc.line(x,y,x+flip*10,y+30);for(let n=5;n<27;n+=6){doc.line(x+flip*n/3,y+n,x+flip*(n/3+5),y+n-5);doc.ellipse(x+flip*(n/3+6),y+n-6,3,1.4,'S')}doc.setFillColor(...gold);for(let n=0;n<4;n++)doc.circle(x+flip*(12+n*3),y+8+n*5,1,'F')}};
    const addLogo=async()=>{if(!s.logo)return;try{const logoData=s.logo.startsWith('data:')?s.logo:await fetch(s.logo).then(r=>r.blob()).then(blob=>new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(blob)}));doc.addImage(logoData,undefined,15,13,30,30,'logo','FAST')}catch(error){console.warn('Logo konnte nicht ins PDF eingefügt werden:',error)}};
    decorate();await addLogo();doc.setTextColor(...ink);doc.setFont('helvetica','bold');doc.setFontSize(17);doc.text(String(s.name||'ATELIER WUFFLI').toUpperCase(),50,23);doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setCharSpace(2.2);doc.text('FLORALES UND MEHR',50,29);doc.setCharSpace(0);doc.setDrawColor(...roseStrong);doc.line(50,34,92,34);
    if(isReceipt){
      doc.setFont('times','italic');doc.setFontSize(32);doc.text('Quittung',105,58,{align:'center'});doc.setFont('helvetica','normal');doc.setFontSize(9);doc.setCharSpace(3);doc.text('ZAHLUNG ERHALTEN',105,69,{align:'center'});doc.setCharSpace(0);doc.setFillColor(255,249,249);doc.roundedRect(15,78,180,31,3,3,'F');doc.setFontSize(10);doc.text('Belegnummer:',21,88);doc.text(String(d.number),64,88);doc.text('Datum:',21,97);doc.text(date(d.date),64,97);doc.text('Rechnung:',112,88);doc.text(String(d.invoiceNumber||'–'),145,88);doc.text('Auftrag:',112,97);doc.text(String(d.orderNumber||'–'),145,97);
    }else{
      doc.setFont('times','italic');doc.setFontSize(29);doc.text('Rechnung',190,54,{align:'right'});doc.setFillColor(...rose);doc.roundedRect(15,62,72,11,5,5,'F');doc.setFont('times','italic');doc.setFontSize(16);doc.text('Rechnungsadresse',20,70);doc.setFont('helvetica','normal');doc.setFontSize(10);const customer=[d.customerSnapshot?.name,d.customerSnapshot?.billing?.street,[d.customerSnapshot?.billing?.zip,d.customerSnapshot?.billing?.city].filter(Boolean).join(' ')].filter(Boolean);doc.text(customer.map(String),20,82);doc.setDrawColor(...roseStrong);doc.line(102,64,102,99);doc.setFontSize(9);doc.setTextColor(...muted);doc.text('Rechnungsnummer:',111,72);doc.text('Rechnungsdatum:',111,80);doc.text('Fällig am:',111,88);doc.text('Kundennummer:',111,96);doc.setTextColor(...ink);doc.text(String(d.number),154,72);doc.text(date(d.date),154,80);doc.text(date(d.dueDate),154,88);doc.text(String(d.customerSnapshot?.number||'–'),154,96);
    }
    let y=isReceipt?119:112;doc.setFillColor(...rose);doc.roundedRect(15,y,180,10,2,2,'F');doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text('Beschreibung',20,y+6.5);doc.text('Menge',130,y+6.5);doc.text('Einzelpreis',151,y+6.5);doc.text('Betrag',190,y+6.5,{align:'right'});y+=12;doc.setFont('helvetica','normal');
    for(const item of d.items){const lines=doc.splitTextToSize(String(item.description||''),102),height=Math.max(9,lines.length*4.5+3);if(y+height>238){doc.addPage();decorate();y=24;doc.setFillColor(...rose);doc.roundedRect(15,y,180,10,2,2,'F');doc.setFont('helvetica','bold');doc.text('Beschreibung',20,y+6.5);doc.text('Menge',130,y+6.5);doc.text('Einzelpreis',151,y+6.5);doc.text('Betrag',190,y+6.5,{align:'right'});doc.setFont('helvetica','normal');y+=12}doc.text(lines,20,y+4);doc.text(String(item.quantity),130,y+4);doc.text(money(item.price),151,y+4);doc.text(money(item.total),190,y+4,{align:'right'});doc.setDrawColor(224,191,194);doc.line(15,y+height,195,y+height);y+=height}
    y+=7;doc.setDrawColor(...roseStrong);doc.line(112,y,195,y);y+=9;doc.setFont('helvetica','bold');doc.setFontSize(11);doc.text(isReceipt?'Gesamtbetrag':'Gesamtbetrag',116,y);doc.setFontSize(16);doc.text(money(d.total),190,y,{align:'right'});y+=7;if(isReceipt){doc.setFillColor(...rose);doc.roundedRect(112,y,83,12,3,3,'F');doc.setFontSize(12);doc.text('Bezahlt',117,y+8);doc.text(money(d.total),190,y+8,{align:'right'});doc.setFont('times','italic');doc.setFontSize(26);doc.text('Danke!',105,264,{align:'center'});doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setCharSpace(2);doc.text('SCHÖN, DASS DU DA BIST.',105,273,{align:'center'});doc.setCharSpace(0)}else{doc.setFillColor(255,249,249);doc.roundedRect(15,246,75,25,3,3,'F');doc.setFont('times','italic');doc.setFontSize(20);doc.text('Vielen Dank!',52,258,{align:'center'});doc.setFont('helvetica','normal');doc.setFontSize(8);doc.text('Wir freuen uns, dass du bei uns bist.',52,265,{align:'center'});doc.setFontSize(8);doc.setTextColor(...muted);doc.text(`Zahlbar bis ${date(d.dueDate)}`,105,250);doc.text(`IBAN: ${s.iban||'–'}`,105,257);doc.text(`Referenz: ${d.number}`,105,264);if(d.text)doc.text(doc.splitTextToSize(String(d.text),80),105,271)}
...
printDocument=function(type,id){
  ...
  const overlay=document.createElement('div');overlay.id='mobile-print-view';overlay.className='mobile-print-view';overlay.innerHTML=`<div class="print-controls"><button class="secondary" id="close-print">Zurück zur App</button><button class="primary" id="start-print">Drucken / als PDF speichern</button></div><div class="print-sheet"><header><div>${s.logo?`<img src="${s.logo}">`:''}<p><strong>${esc(s.name)}</strong><br>${docAddress(s.address)}</p></div><div><strong>${esc(d.customerSnapshot.name)}</strong><br>${esc(d.customerSnapshot.billing.street||'')}<br>${esc([d.customerSnapshot.billing.zip,d.customerSnapshot.billing.city].filter(Boolean).join(' '))}</div></header><h1>${isReceipt?'Quittung':isInv?'Rechnung':'Auftrag'} ${esc(d.number)}</h1><p>Datum: ${date(d.date)}${isReceipt?`<br>Rechnung: ${esc(d.invoiceNumber)}<br>Auftrag: ${esc(d.orderNumber||'–')}`:''}</p>${fulfil}<table><thead><tr><th>Beschreibung</th><th>Menge</th><th>Einzelpreis</th><th>Betrag</th></tr></thead><tbody>${rows}</tbody></table><p class="total">Gesamtbetrag: ${money(d.total)}</p><div class="footer">${docAddress(d.text)}${isReceipt?`<p><strong>Der Rechnungsbetrag wurde vollständig bezahlt.</strong></p>`:isInv?`<p>Zahlbar bis ${date(d.dueDate)}<br>IBAN: ${esc(s.iban)}<br>Referenz: ${esc(d.number)}</p>`:''}</div></div>`;
