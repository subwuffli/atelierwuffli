...
invoiceForm=async function(id){
  ...
  const invoiceFormElement=$('#invoice-form');if(invoiceFormElement){const paidDateField=invoiceFormElement.querySelector('[name="paidDate"]')?.closest('label');paidDateField?.insertAdjacentHTML('afterend',`<label>Zahlungsart<select name="paymentMethod"><option value="" ${!state.invoices.find(x=>x.id===id)?.paymentMethod?'selected':''}>Nicht angegeben</option><option ${state.invoices.find(x=>x.id===id)?.paymentMethod==='Überweisung'?'selected':''}>Überweisung</option><option ${state.invoices.find(x=>x.id===id)?.paymentMethod==='Barzahlung'?'selected':''}>Barzahlung</option><option ${state.invoices.find(x=>x.id===id)?.paymentMethod==='TWINT'?'selected':''}>TWINT</option><option ${state.invoices.find(x=>x.id===id)?.paymentMethod==='Karte'?'selected':''}>Karte</option></select></label>`)}
};
...
async function pdfDocument(type,id){
  ...
  const {jsPDF}=window.jspdf,doc=new jsPDF({unit:'mm',format:'a4'}),s=state.settings,isInv=type==='invoice',isReceipt=type==='receipt';
  {
    ...
    doc.setFont('helvetica','normal');doc.setFontSize(7);doc.setTextColor(...muted);doc.text(doc.splitTextToSize([businessName(s),businessAddress(s)].filter(Boolean).join(' · '),105),20,25);
