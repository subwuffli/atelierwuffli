...
async function pdfMonthlyReport(kind,month){
  ...
  const isExpense=kind==='expenses',rows=isExpense?state.expenses.filter(x=>monthKey(x.date)===month):state.invoices.filter(i=>i.status==='Bezahlt'&&monthKey(incomeDate(i))===month),total=rows.reduce((sum,x)=>sum+Number(isExpense?x.amount:x.total),0),{jsPDF}=window.jspdf,doc=new jsPDF({unit:'mm',format:'a4'}),s=state.settings,rose=[252,229,229],roseStrong=[235,151,151],ink=[54,47,52],muted=[105,86,94];
  const decorate=()=>{doc.setFillColor(255,255,255);doc.rect(0,0,210,297,'F');doc.setDrawColor(...roseStrong);doc.setLineWidth(.35);doc.roundedRect(7,7,196,283,3,3,'S')};
  const addLogo=async()=>{if(!s.logo)return;try{doc.addImage(await preparePdfLogo(s.logo),'PNG',15,13,42,42,'report-logo','FAST')}catch(error){console.warn('Logo konnte nicht ins PDF eingefügt werden:',error)}};
  decorate();await addLogo();doc.setTextColor(...ink);doc.setFont('times','italic');doc.setFontSize(28);doc.text(isExpense?'Ausgaben':'Einnahmen',190,42,{align:'right'});doc.setFont('helvetica','normal');doc.setFontSize(10);doc.setTextColor(...muted);doc.text(monthLabel(month),190,50,{align:'right'});doc.text(`Erstellt am ${date(today())}`,190,56,{align:'right'});
  let y=72;doc.setFillColor(...rose);doc.roundedRect(15,y,180,10,2,2,'F');doc.setTextColor(...ink);doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text('Datum',20,y+6.5);doc.text(isExpense?'Beschreibung':'Rechnung / Kunde',48,y+6.5);doc.text('Betrag',190,y+6.5,{align:'right'});y+=12;doc.setFont('helvetica','normal');
...
async function pdfDocument(type,id){
  ...
  const {jsPDF}=window.jspdf,doc=new jsPDF({unit:'mm',format:'a4'}),s=state.settings,isInv=type==='invoice',isReceipt=type==='receipt';
  {
    ...
    if(isReceipt){
      ...
      doc.setFont('times','italic');doc.setFontSize(29);doc.text('Auftrag',190,54,{align:'right'});doc.setFillColor(...rose);doc.roundedRect(15,62,72,11,5,5,'F');doc.setFont('times','italic');doc.setFontSize(16);doc.text('Kundenadresse',20,70);doc.setFont('helvetica','normal');doc.setFontSize(10);const customer=[d.customerSnapshot?.name,d.customerSnapshot?.billing?.street,[d.customerSnapshot?.billing?.zip,d.customerSnapshot?.billing?.city].filter(Boolean).join(' ')].filter(Boolean);doc.text(customer.map(String),20,82);doc.setDrawColor(...roseStrong);doc.line(102,64,102,99);doc.setFontSize(9);doc.setTextColor(...muted);doc.text('Auftragsnummer:',111,72);doc.text('Auftragsdatum:',111,80);doc.text(`${d.fulfilment||'Erfüllung'}:`,111,88);doc.text('Kundennummer:',111,96);doc.setTextColor(...ink);doc.text(String(d.number),154,72);doc.text(date(d.date),154,80);doc.text(date(d.fulfilmentDate),154,88);doc.text(String(d.customerSnapshot?.number||'–'),154,96);
    }
    ...
    const companyLines=[String(s.name||''),...String(s.address||'').split(/\r?\n/).filter(Boolean)];
    if(isReceipt){doc.setFillColor(...rose);doc.roundedRect(112,y,83,12,3,3,'F');doc.setFontSize(12);doc.text('Bezahlt',117,y+8);doc.text(money(d.total),190,y+8,{align:'right'});doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text('Quittungsinformationen',20,215);doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(...muted);doc.text([...companyLines,`Rechnung: ${d.invoiceNumber||'–'}`,`Bezahlt am ${date(d.date)}`],20,223)}
    else if(isInv){doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text('Rechnungsinformationen',20,198);doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(...muted);doc.text([...companyLines,`Zahlbar bis ${date(d.dueDate)}`,`IBAN: ${s.iban||'–'}`,`Referenz: ${d.number}`],20,206)}
    else{doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text('Auftragsinformationen',20,198);doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(...muted);const info=[...companyLines,`${d.fulfilment||'Erfüllung'} am ${date(d.fulfilmentDate)}`];if(d.text)info.push(...doc.splitTextToSize(String(d.text),75));doc.text(info,20,206)}
...
printDocument=async function(type,id){
  ...
  const printLogo=s.logo?await preparePdfLogo(s.logo).catch(()=>s.logo):'',info=isReceipt?`<strong>Quittungsinformationen</strong><br>${esc(s.name)}<br>${docAddress(s.address)}<br>Rechnung: ${esc(d.invoiceNumber)}<br>Bezahlt am ${date(d.date)}`:isInv?`<strong>Rechnungsinformationen</strong><br>${esc(s.name)}<br>${docAddress(s.address)}<br>Zahlbar bis ${date(d.dueDate)}<br>IBAN: ${esc(s.iban)}<br>Referenz: ${esc(d.number)}`:`<strong>Auftragsinformationen</strong><br>${esc(s.name)}<br>${docAddress(s.address)}<br>${esc(d.fulfilment)} am ${date(d.fulfilmentDate)}`;
