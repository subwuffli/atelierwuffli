...
const APP_VERSION='TEST V0.0.94.0';
...
function groupedBarChart(rows,years,title){const max=Math.max(...rows.flatMap(row=>row.values),1);return `<div class="card finance-chart"><h2>${esc(title)}</h2><div class="chart-legend">${years.map((year,index)=>`<span><i class="year-${index}"></i>${year}</span>`).join('')}</div><div class="bar-chart grouped-bar-chart" style="--bar-count:${rows.length}">${rows.map(row=>`<div class="bar-group">${row.values.map((value,index)=>`<div class="bar-series"><span class="bar-value">${value||'–'}</span><div class="bar year-${index}" style="height:${Math.max(value?6:0,value/max*100)}%"></div></div>`).join('')}<span class="bar-label">${esc(row.label)}</span></div>`).join('')}</div></div>`}
