...
const APP_VERSION='TEST V0.0.62.0';
...
const userPreferences=()=>({customerSort,orderSort,invoiceSort,receiptSort,financeMonth,appointmentView,appointmentMonth,listColumns,positionTemplates});
...
function openPositionTemplates(){
  const templates=structuredClone(positionTemplates);
  ...
  $('#position-template-form').onsubmit=async event=>{event.preventDefault();if(templates.some(template=>!String(template.name||'').trim())){alert('Bitte gib jeder Vorlage eine Bezeichnung.');return}positionTemplates=templates.map(template=>({...template,name:String(template.name).trim(),price:Number(template.price)||0,active:template.active!==false}));try{await saveUserPreferences();await closeModal();renderCloudSettings();notice('Positionsvorlagen gespeichert.')}catch(error){alert(`Vorlagen konnten nicht gespeichert werden: ${error.message}`)}};
}
