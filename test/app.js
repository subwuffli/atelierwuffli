...
const APP_VERSION='TEST V0.0.39.0';
...
function openMoreMenu(){const destinations=[['customers','Kunden'],['orders','Aufträge'],['invoices','Rechnungen'],['receipts','Quittungen'],['expenses','Ausgaben'],['income','Einnahmen'],['settings','Einstellungen']];modal('Mehr',`<div class="more-grid">${destinations.map(([view,label])=>`<button type="button" class="secondary" data-more-view="${view}">${label}</button>`).join('')}</div>`);$('#modal-body').onclick=e=>{const view=e.target.closest('[data-more-view]')?.dataset.moreView;if(view){closeModal();render(view)}}}
...
function bindGlobal(){
  ...
  $('#menu-button').onclick=()=>window.matchMedia('(max-width:900px)').matches?openMoreMenu():$('.sidebar').classList.toggle('open');
