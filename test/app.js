...
const APP_VERSION='TEST V0.0.56.0';
...
function splitStreetAndHouseNumber(value=''){const match=String(value||'').trim().match(/^(.*?)(?:[\s,]+)(\d+[a-zA-Z]?(?:[-/]\d+[a-zA-Z]?)?)$/);return match?{street:match[1].trim(),houseNumber:match[2]}:{street:String(value||'').trim(),houseNumber:''}}
function addressAutocompleteMarkup({idPrefix,street='',zip='',city='',streetName='street',zipName='zip',cityName='city',houseNumberName='houseNumber',streetId='',zipId='',cityId='',houseNumberId=''}){const address=splitStreetAndHouseNumber(street),id=name=>`${idPrefix}-${name}`;return `<div class="address-autocomplete span-2" data-address-autocomplete><label class="span-2">PLZ oder Ort<input id="${id('locality')}" type="search" autocomplete="off" placeholder="z. B. 4614 oder Hägendorf" aria-autocomplete="list" aria-controls="${id('locality-results')}"></label><div id="${id('locality-results')}" class="address-suggestions hidden" role="listbox" aria-label="PLZ- und Ortsvorschläge"></div><div class="address-autocomplete-grid"><label>PLZ<input ${zipId?`id="${zipId}"`:''} data-address-zip name="${zipName}" inputmode="numeric" autocomplete="postal-code" value="${esc(zip)}"></label><label>Ort<input ${cityId?`id="${cityId}"`:''} data-address-city name="${cityName}" autocomplete="address-level2" value="${esc(city)}"></label><label>Strasse<input ${streetId?`id="${streetId}"`:''} data-address-street name="${streetName}" autocomplete="address-line1" value="${esc(address.street)}" ${zip?'':'disabled'}></label><label>Hausnummer<input ${houseNumberId?`id="${houseNumberId}"`:''} data-address-house-number name="${houseNumberName}" autocomplete="address-line2" value="${esc(address.houseNumber)}"></label></div><p class="address-autocomplete-hint muted">Ort und Strasse werden live über OpenPLZ gesucht. Hausnummer frei eingeben.</p><div id="${id('street-results')}" class="address-suggestions hidden" role="listbox" aria-label="Strassenvorschläge"></div></div>`}
function mountAddressAutocomplete(root){const locality=root.querySelector('input[type="search"]'),zip=root.querySelector('[data-address-zip]'),city=root.querySelector('[data-address-city]'),street=root.querySelector('[data-address-street]'),localityResults=root.querySelector('[id$="locality-results"]'),streetResults=root.querySelector('[id$="street-results"]');let selectedPostalCode=zip.value.trim(),localityController=null,streetController=null;const hide=element=>{element.classList.add('hidden');element.innerHTML=''};const show=(element,items,render,select)=>{element.innerHTML=items.map((item,index)=>`<button type="button" role="option" data-address-choice="${index}">${render(item)}</button>`).join('');element.classList.toggle('hidden',!items.length);element.querySelectorAll('[data-address-choice]').forEach(button=>button.addEventListener('click',()=>select(items[Number(button.dataset.addressChoice)])))};const request=async(path,kind)=>{const previous=kind==='locality'?localityController:streetController;previous?.abort();const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),7000);if(kind==='locality')localityController=controller;else streetController=controller;try{const response=await fetch(`${OPENPLZ_API_BASE}${path}`,{signal:controller.signal,headers:{Accept:'application/json'}});if(!response.ok)throw new Error(`OpenPLZ ${response.status}`);return await response.json()}catch(error){if(error.name!=='AbortError')console.warn('OpenPLZ nicht erreichbar:',error);return []}finally{clearTimeout(timeout);if(kind==='locality'&&localityController===controller)localityController=null;if(kind==='street'&&streetController===controller)streetController=null}};const queryLocalities=debounce(async()=>{const value=locality.value.trim();if(value.length<2){hide(localityResults);return}const parameter=/\d/.test(value)?'postalCode':'name',rows=await request(`/ch/Localities?${parameter}=${encodeURIComponent(value)}&pageSize=12`,'locality');show(localityResults,rows,item=>`${esc(item.postalCode)} ${esc(item.name)}`,item=>{selectedPostalCode=String(item.postalCode||'');zip.value=selectedPostalCode;city.value=item.name||'';locality.value=`${selectedPostalCode} ${city.value}`.trim();street.disabled=!selectedPostalCode;hide(localityResults);hide(streetResults);street.focus()})},250);const queryStreets=debounce(async()=>{const value=street.value.trim();if(!selectedPostalCode||value.length<2){hide(streetResults);return}const pattern=value.startsWith('^')?value:`^${value}`,rows=await request(`/ch/Streets?name=${encodeURIComponent(pattern)}&postalCode=${encodeURIComponent(selectedPostalCode)}&pageSize=12`,'street');show(streetResults,rows,item=>`${esc(item.name)} <span>${esc(item.postalCode||selectedPostalCode)} ${esc(item.locality||'')}</span>`,item=>{street.value=item.name||'';hide(streetResults)})},250);locality.addEventListener('input',queryLocalities);street.addEventListener('input',queryStreets);zip.addEventListener('input',()=>{selectedPostalCode='';street.disabled=true;hide(streetResults)});root.addEventListener('focusout',()=>setTimeout(()=>{hide(localityResults);hide(streetResults)},160));}
...
function openDocumentFromRow(view,row){const cellText=row?.firstElementChild?.textContent.trim();if(!cellText)return;if(view==='orders'){const order=state.orders.find(entry=>cellText.startsWith(entry.number));if(order)startDocumentDetail('order',order.id,view)}else if(view==='invoices'){const invoice=state.invoices.find(entry=>cellText.startsWith(entry.number));if(invoice)startDocumentDetail('invoice',invoice.id,view)}else if(view==='receipts'){const invoice=state.invoices.find(entry=>entry.receipt?.number&&cellText.startsWith(entry.receipt.number));if(invoice)startDocumentDetail('receipt',invoice.id,view)}}
...
function setAppointmentView(view){appointmentView=view;renderAppointments()}
function moveAppointmentMonth(offset){const [year,month]=appointmentMonth.split('-').map(Number),next=new Date(year,month-1+offset,1);appointmentMonth=`${next.getFullYear()}-${String(next.getMonth()+1).padStart(2,'0')}`;renderAppointments()}
...
customerForm=async function(id){
  ...
  const existingStreet=form.elements.street,existingZip=form.elements.zip,existingCity=form.elements.city,addressHost=document.createElement('div');addressHost.className='span-2';
  addressHost.innerHTML=addressAutocompleteMarkup({idPrefix:'customer-address',street:existingStreet?.value||'',zip:existingZip?.value||'',city:existingCity?.value||''});
  existingStreet?.closest('label')?.before(addressHost);existingStreet?.closest('label')?.remove();existingZip?.closest('label')?.remove();existingCity?.closest('label')?.remove();mountAddressAutocomplete(addressHost);
  form.onsubmit=async event=>{
    event.preventDefault();const submit=form.querySelector('button.primary'),values=Object.fromEntries(new FormData(form)),houseNumber=String(values.houseNumber||'').trim();delete values.houseNumber;values.street=[String(values.street||'').trim(),houseNumber].filter(Boolean).join(' ');const deliveries=[...form.querySelectorAll('#delivery-list .line-item')].map(row=>({id:uid(),label:row.querySelector('[data-d="label"]')?.value.trim()||'',street:row.querySelector('[data-d="street"]')?.value.trim()||'',city:row.querySelector('[data-d="city"]')?.value.trim()||''})),candidate={...(existing||{}),...values,id:existing?.id||uid(),number:existing?.number||'',deliveries,archived:existing?.archived||false,createdAt:existing?.createdAt||new Date().toISOString()};
...
orderForm=async function(id){
  ...
  if(form){
    ...
    customerField?.insertAdjacentHTML('afterend',`<div id="new-customer-panel" class="span-2 inline-create-panel hidden"><div class="inline-create-head"><strong>Neuen Kunden erfassen</strong><button type="button" class="text-button" id="cancel-new-customer">Schliessen</button></div><div class="form-grid compact-grid"><label>Firma<input id="inline-company"></label><label>Anrede<input id="inline-salutation"></label><label>Vorname<input id="inline-first-name"></label><label>Nachname<input id="inline-last-name"></label><label>E-Mail<input id="inline-email" type="email"></label><label>Telefon<input id="inline-phone" type="tel"></label>${addressAutocompleteMarkup({idPrefix:'inline-customer-address',streetId:'inline-street',zipId:'inline-zip',cityId:'inline-city',houseNumberId:'inline-house-number'})}</div><div id="inline-customer-error" class="error small"></div><div class="actions inline-create-actions"><button type="button" class="primary" id="save-new-customer">Kunden speichern und auswählen</button></div></div>`);
    $('#inline-salutation').outerHTML=salutationSelect('','inline-salutation');
    mountAddressAutocomplete($('#new-customer-panel'));
