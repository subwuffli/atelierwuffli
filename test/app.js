...
const APP_VERSION='TEST V0.0.99.0';
...
function listTable(view,rows,renderCells,renderActions,{documentType='',recordId=row=>row.id,compactDocument=false}={}){const columns=listColumns[view]||defaultListColumns()[view],headers=columns.map(id=>`<th>${esc(listOptionLabel(view,id))}</th>`).join(''),actionHeader=compactDocument?'<th>Öffnen</th>':'<th></th>';return rows.length?`<div class="table-wrap"><table><thead><tr>${headers}${actionHeader}</tr></thead><tbody>${rows.map(row=>`<tr${documentType?` data-document-type="${documentType}" data-record-id="${esc(recordId(row))}" tabindex="0"`:''}${compactDocument?' data-compact-document="true"':''}>${columns.map(id=>`<td data-label="${esc(listOptionLabel(view,id))}">${renderCells(row,id)}</td>`).join('')}${compactDocument?'<td data-label="Öffnen" class="document-open-hint">Öffnen ›</td>':`<td data-label="Aktionen">${renderActions(row)}</td>`}</tr>`).join('')}</tbody></table></div>`:''}
...
showDocumentDetail=function(type,id){
  ...
  const more=type==='order'?`<details class="document-more-actions"><summary>Weitere Aktionen</summary><div><button type="button" class="secondary" onclick="toggleArchive('orders','${order.id}')">${order.archived?'Aktivieren':'Archivieren'}</button><button type="button" class="danger" onclick="deleteRecord('order','${order.id}','${esc(order.number)}')">Löschen</button></div></details>`:type==='invoice'?`<details class="document-more-actions"><summary>Weitere Aktionen</summary><div><button type="button" class="secondary" onclick="toggleArchive('invoices','${invoice.id}')">${invoice.archived?'Aktivieren':'Archivieren'}</button><button type="button" class="danger" onclick="deleteRecord('invoice','${invoice.id}','${esc(invoice.number)}')">Löschen</button></div></details>`:`<details class="document-more-actions"><summary>Weitere Aktionen</summary><div><button type="button" class="danger" onclick="deleteRecord('receipt','${receipt.id}','${esc(receipt.number)}')">Löschen</button></div></details>`;
  const orderMore=type==='order'?more.replace(order.archived?'Aktivieren':'Archivieren',order.archived?'Einblenden':'Ausblenden'):more;
  toolbar.querySelector('.actions').innerHTML=actions+orderMore;
...
function orderStatistics(orders,month=statisticsMonth,year=new Date().getFullYear()){
  const current=orders.filter(order=>monthKey(order.date)===month),years=[year,year-1,year-2];
  return {month,years,open:current.filter(order=>order.status==='In Arbeit').length,completed:current.filter(order=>order.status==='Abgeschlossen').length,total:current.length,months:Array.from({length:12},(_,index)=>({label:monthName(index),values:years.map(value=>orders.filter(order=>monthKey(order.date)===`${value}-${String(index+1).padStart(2,'0')}`).length)}))};
}
function statisticsMonthOptions(selected){return lastMonths(36).reverse().map(month=>`<option value="${month}" ${month===selected?'selected':''}>${esc(monthLabel(month))}</option>`).join('')}
function renderStatistics(){
  const data=orderStatistics(state.orders);
  setTitle('Statistik');
  $('#content').innerHTML=`<div class="section-head"><div><h2>Auftragsstatistik</h2><p class="muted">Auftragsstatus nach Monat sowie Vergleich der letzten drei Kalenderjahre.</p></div><label>Monat<select id="statistics-month">${statisticsMonthOptions(data.month)}</select></label></div><div class="grid stats finance-stats"><button type="button" class="card stat dashboard-link" onclick="render('orders')"><span class="muted">Offene Aufträge ${monthLabel(data.month)}</span><strong>${data.open}</strong><span class="dashboard-link-hint">Aufträge öffnen →</span></button><button type="button" class="card stat dashboard-link" onclick="render('orders')"><span class="muted">Abgeschlossene Aufträge ${monthLabel(data.month)}</span><strong>${data.completed}</strong><span class="dashboard-link-hint">Aufträge öffnen →</span></button><div class="card stat"><span class="muted">Alle Aufträge ${monthLabel(data.month)}</span><strong>${data.total}</strong></div></div>${horizontalBarChart([{label:'Offen',value:data.open},{label:'Abgeschlossen',value:data.completed}],`Auftragsstatus ${monthLabel(data.month)}`)}${horizontalGroupedBarChart(data.months,data.years,'Aufträge pro Monat · Jahresvergleich')}`;
  $('#statistics-month').onchange=event=>{statisticsMonth=event.target.value;renderStatistics()};
}
...
function renderAppointmentCalendar(rows){
  const [year,month]=appointmentMonth.split('-').map(Number),first=new Date(year,month-1,1),days=new Date(year,month,0).getDate(),leading=(first.getDay()+6)%7,monthLabel=new Intl.DateTimeFormat('de-CH',{month:'long',year:'numeric'}).format(first),counts=new Map();
  rows.forEach(order=>{if(order.fulfilmentDate.startsWith(appointmentMonth))counts.set(order.fulfilmentDate,(counts.get(order.fulfilmentDate)||0)+1)});
  const cells=Array.from({length:leading},()=>'<div class="calendar-day calendar-day-empty" aria-hidden="true"></div>');
...
function renderAppointments(){
  ...
  if(appointmentView==='calendar'){$('#content').innerHTML=`<div class="section-head appointment-view-head"><div><h2>Terminübersicht</h2><p class="muted">Markierte Tage enthalten offene Termine.</p></div>${appointmentControls()}</div>${renderAppointmentCalendar(rows)}`;return}
...
function invoicesTable(rows){const cells=(i,id)=>({number:`${esc(i.number)}<br><span class="muted">${date(i.date)}</span>${i.receipt?`<br><span class="muted">Quittung ${esc(i.receipt.number)}</span>`:''}`,customer:esc(i.customerSnapshot?.name),due:date(i.dueDate),status:`<span class="badge ${isOverdue(i)?'danger':i.status==='Bezahlt'?'ok':i.status==='Storniert'?'danger':'warn'}">${isOverdue(i)?'Überfällig':esc(i.status)}</span>`,total:money(i.total)})[id]||'';return listTable('invoices',rows,cells,()=>'',{documentType:'invoice',compactDocument:true})||'<div class="card empty">Noch keine Rechnungen vorhanden.</div>'}
function receiptsTable(rows){const cells=({invoice,receipt},id)=>({number:esc(receipt.number),invoice:esc(receipt.invoiceNumber||invoice.number),customer:esc(receipt.customerSnapshot?.name),date:date(receipt.date),total:money(receipt.total)})[id]||'';return listTable('receipts',rows,cells,()=>'',{documentType:'receipt',recordId:row=>row.invoice.id,compactDocument:true})||'<div class="card empty">Noch keine Quittungen vorhanden.</div>'}
...
const monthName=index=>new Intl.DateTimeFormat('de-CH',{month:'short'}).format(new Date(2020,index,1));
function lastMonths(count=12,end=new Date()){const months=[];for(let n=count-1;n>=0;n--){const d=new Date(end.getFullYear(),end.getMonth()-n,1);months.push(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`)}return months}
function barChart(points,title,valueLabel=value=>value){const max=Math.max(...points.map(point=>Number(point.value)||0),1);return `<div class="card finance-chart"><h2>${esc(title)}</h2><div class="bar-chart" style="--bar-count:${points.length}">${points.map(point=>{const value=Number(point.value)||0;return `<div class="bar-column"><span class="bar-value">${esc(value?valueLabel(value):'–')}</span><div class="bar" style="height:${Math.max(value?6:0,value/max*100)}%"></div><span class="bar-label">${esc(point.label)}</span></div>`}).join('')}</div></div>`}
function horizontalBarChart(points,title){const max=Math.max(...points.map(point=>Number(point.value)||0),1);return `<div class="card finance-chart horizontal-chart"><h2>${esc(title)}</h2><div class="horizontal-bars">${points.map(point=>{const value=Number(point.value)||0;return `<div class="horizontal-row"><span>${esc(point.label)}</span><div class="horizontal-track"><div class="bar" style="width:${value/max*100}%"></div></div><strong>${value}</strong></div>`}).join('')}</div></div>`}
function horizontalGroupedBarChart(rows,years,title){const max=Math.max(...rows.flatMap(row=>row.values),1);return `<div class="card finance-chart horizontal-chart"><h2>${esc(title)}</h2><div class="chart-legend">${years.map((year,index)=>`<span><i class="year-${index}"></i>${year}</span>`).join('')}</div><div class="horizontal-months">${rows.map(row=>`<div class="horizontal-month"><strong>${esc(row.label)}</strong><div>${row.values.map((value,index)=>`<div class="horizontal-series"><i class="year-${index}"></i><div class="horizontal-track"><div class="bar year-${index}" style="width:${value/max*100}%"></div></div><span>${value}</span></div>`).join('')}</div></div>`).join('')}</div></div>`}
function financeChart(entries,dateOf,amountOf){const months=lastMonths(),totals=months.map(month=>entries.filter(entry=>monthKey(dateOf(entry))===month).reduce((sum,entry)=>sum+Number(amountOf(entry)||0),0));return barChart(months.map((month,index)=>({label:monthLabel(month),value:totals[index]})),'Letzte 12 Monate',money)}
...
function openPositionTemplates(){
  const templates=structuredClone(state.settings.positionTemplates||[]);
  const draw=()=>{$('#position-template-list').innerHTML=templates.length?templates.map((template,index)=>`<div class="line-item template-row"><label>Bezeichnung<input data-template="name" data-index="${index}" value="${esc(template.name)}" required></label><label>Preis CHF<input data-template="price" data-index="${index}" type="number" min="0" step="0.01" value="${Number(template.price)||0}" required></label><label class="inline">Aktiv<input data-template="active" data-index="${index}" type="checkbox" ${template.active!==false?'checked':''}></label><button type="button" class="danger" data-remove-template="${index}" aria-label="Vorlage entfernen">×</button></div>`).join(''):'<p class="hint">Noch keine Positionsvorlagen vorhanden.</p>'};
  modal('Positionsvorlagen',`<form id="position-template-form"><p class="hint">Vorlagen werden nur beim Einfügen in den Auftrag verwendet. Bereits gespeicherte Positionen bleiben unverändert.</p><div id="position-template-list"></div><button type="button" class="secondary" id="add-position-template">Vorlage hinzufügen</button><div class="form-actions"><button type="button" class="secondary" id="cancel-position-templates">Abbrechen</button><button class="primary">Speichern</button></div></form>`);
  draw();
  $('#position-template-list').oninput=event=>{const field=event.target.dataset.template;if(!field)return;const template=templates[Number(event.target.dataset.index)];template[field]=field==='price'?Number(event.target.value):event.target.value};
  $('#position-template-list').onchange=event=>{if(event.target.dataset.template==='active')templates[Number(event.target.dataset.index)].active=event.target.checked};
  $('#position-template-list').onclick=event=>{if(event.target.dataset.removeTemplate!==undefined){templates.splice(Number(event.target.dataset.removeTemplate),1);draw()}};
  $('#add-position-template').onclick=()=>{templates.push({id:uid(),name:'',price:0,active:true});draw()};
...
toggleArchive=async function(kind,id){
  ...
  try{if(!(await acquireEditLock(entityType,id))){alert(editLockConflictMessage(kind==='customers'?'Dieser Kunde':kind==='orders'?'Dieser Auftrag':'Diese Rechnung'));return}state=await loadFromSupabase();record=state[kind].find(x=>x.id===id);if(kind==='orders'&&!record.archived&&record.status!=='Abgeschlossen')throw new Error('Nur abgeschlossene Aufträge können ausgeblendet werden.');const candidate={...record,archived:!record.archived};if(kind==='customers')await saveCustomerRecord(candidate,record.updatedAt,true);else if(kind==='orders')await saveOrderRecord(candidate,record.updatedAt,true);else await saveInvoiceRecord(candidate,record.updatedAt,true);notice(kind==='orders'?(candidate.archived?'Auftrag ausgeblendet.':'Auftrag eingeblendet.'):(candidate.archived?'Archiviert.':'Wieder aktiviert.'))}catch(error){alert(`${kind==='orders'?'Ausblenden':'Archivstatus'} konnte nicht gespeichert werden: ${error.message}`)}finally{await releaseCurrentEditLock()}render(currentView)
};
