...
const APP_VERSION='TEST V0.0.97.0';
...
showDocumentDetail=function(type,id){
  ...
  toolbar.querySelector('.actions').innerHTML=actions+more;
...
function orderStatistics(orders,month=statisticsMonth,year=new Date().getFullYear()){
  const active=orders.filter(order=>!order.archived),current=active.filter(order=>monthKey(order.date)===month),years=[year,year-1,year-2];
  return {month,years,open:current.filter(order=>order.status==='In Arbeit').length,completed:current.filter(order=>order.status==='Abgeschlossen').length,total:current.length,months:Array.from({length:12},(_,index)=>({label:monthName(index),values:years.map(value=>active.filter(order=>monthKey(order.date)===`${value}-${String(index+1).padStart(2,'0')}`).length)}))};
}
