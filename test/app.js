...
const APP_VERSION='TEST V0.0.46.0';
...
function openDocumentFromRow(view,row){const cellText=row?.firstElementChild?.textContent.trim();if(!cellText)return;if(view==='orders'){const order=state.orders.find(entry=>cellText.startsWith(entry.number));if(order)openDocumentDetail('order',order.id,view)}else if(view==='invoices'){const invoice=state.invoices.find(entry=>cellText.startsWith(entry.number));if(invoice)openDocumentDetail('invoice',invoice.id,view)}else if(view==='receipts'){const invoice=state.invoices.find(entry=>entry.receipt?.number&&cellText.startsWith(entry.receipt.number));if(invoice)openDocumentDetail('receipt',invoice.id,view)}}
