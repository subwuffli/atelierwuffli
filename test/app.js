...
const APP_VERSION='TEST V0.0.93.0';
...
function orderStatistics(orders,month=today().slice(0,7)){
  const active=orders.filter(order=>!order.archived),current=active.filter(order=>monthKey(order.date)===month),months=lastMonths(36);
  return {month,open:current.filter(order=>order.status==='In Arbeit').length,completed:current.filter(order=>order.status==='Abgeschlossen').length,total:current.length,months:months.map(key=>({label:monthLabel(key),value:active.filter(order=>monthKey(order.date)===key).length}))};
}
function renderStatistics(){
  const data=orderStatistics(state.orders);
  setTitle('Statistik');
  $('#content').innerHTML=`<div class="section-head"><div><h2>Auftragsstatistik</h2><p class="muted">Aktueller Monat und Entwicklung der letzten drei Jahre.</p></div></div><div class="grid stats finance-stats"><button type="button" class="card stat dashboard-link" onclick="render('orders')"><span class="muted">Offene Aufträge ${monthLabel(data.month)}</span><strong>${data.open}</strong><span class="dashboard-link-hint">Aufträge öffnen →</span></button><button type="button" class="card stat dashboard-link" onclick="render('orders')"><span class="muted">Abgeschlossene Aufträge ${monthLabel(data.month)}</span><strong>${data.completed}</strong><span class="dashboard-link-hint">Aufträge öffnen →</span></button><div class="card stat"><span class="muted">Alle Aufträge ${monthLabel(data.month)}</span><strong>${data.total}</strong></div></div>${barChart([{label:'Offen',value:data.open},{label:'Abgeschlossen',value:data.completed}],`Auftragsstatus ${monthLabel(data.month)}`)}${barChart(data.months,'Aufträge pro Monat · letzte 3 Jahre')}`;
}
